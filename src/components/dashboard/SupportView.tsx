...
export const SupportView: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Array<{ sender: 'agent' | 'user'; text: string; time: string }>>([
    { sender: 'agent', text: `Hello ${currentUser.firstName}, welcome to Nova Premier Concierge. How may I assist with your banking requirements today?`, time: '10:02 AM' },
  ]);
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      let reply = "Thank you for contacting Nova. I've noted your inquiry in our priority client log. A senior relationship manager has verified your secure session.";
      const lower = userText.toLowerCase();

      if (lower.includes('card') || lower.includes('limit')) {
        reply = "I see your inquiry regarding card management. You can adjust limits dynamically in the Cards & Limits tab, or I can submit an institutional limit increase request for your Nova Premier card.";
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200/80 dark:border-zinc-800 shadow-sm p-6 space-y-3">
            ...
            <div className="space-y-2 text-xs">
              {[
                ...
                { q: 'What is the maximum daily card limit?', a: 'Nova Premier metal cards feature up to $25,000 daily spend capacity.' },
